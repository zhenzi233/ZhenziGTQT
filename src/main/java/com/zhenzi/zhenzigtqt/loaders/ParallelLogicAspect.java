...
public class ParallelLogicAspect {
    public ParallelLogicAspect() {
    }
    ...
    public static int getMaxRecipeMultiplier(@NotNull AspectRecipe recipe, @NotNull IItemHandlerModifiable inputs, @NotNull IMultipleTankHandler fluidInputs, @NotNull IMultipleAspectTankHandler aspectInputs, int parallelAmount) {
        ...
        boolean itemM = itemMultiplier == Integer.MAX_VALUE;
        boolean fluidM = fluidMultiplier == Integer.MAX_VALUE;
        boolean aspectM = aspectMultiplier == Integer.MAX_VALUE;
        int mina = Math.min(itemMultiplier, fluidMultiplier);
        return itemM && fluidM && aspectM ? 0 : Math.min(mina, aspectMultiplier);
    }

    public static int limitByOutputMerging(@NotNull AspectRecipe recipe, @NotNull IItemHandlerModifiable outputs,
                                           @NotNull IMultipleTankHandler fluidOutputs,
                                           @NotNull IMultipleAspectTankHandler aspectOutputs,
                                           int parallelAmount, boolean voidItems, boolean voidFluids, boolean voidAspects) {
        int modifiedItemParallelAmount = Integer.MAX_VALUE;
        int modifiedFluidParallelAmount = Integer.MAX_VALUE;
        int modifiedAspectParallelAmount = Integer.MAX_VALUE;
        if (voidItems && voidFluids && voidAspects) {
            ...
            if (!recipe.getAspectOutputs().isEmpty() || !recipe.getChancedAspectOutputs().getChancedEntries().isEmpty()) {
                if (voidAspects) {
                    modifiedAspectParallelAmount = parallelAmount;
                } else {
                    modifiedAspectParallelAmount = limitParallelByAspects(recipe, new OverlayedAspectHandler(aspectOutputs), modifiedItemParallelAmount);
                }

                if (modifiedAspectParallelAmount == 0 && !voidAspects) {
                    return 0;
                }
            }

            int mina = Math.min(modifiedFluidParallelAmount, modifiedItemParallelAmount);
            return Math.min(mina, modifiedAspectParallelAmount);
        }
    }
    ...
    public static int limitParallelByItemsIncremental(@NotNull List<ItemStack> recipeOutputList, @NotNull List<ItemStack> outputsToAppend, @NotNull OverlayedItemHandler overlayedItemHandler, int multiplier) {
        ...
        while(minMultiplier != maxMultiplier) {
            ...
            if (currentMultiplier != previousMultiplier) {
                ...
                int finalReturnedAmount = returnedAmount;
                recipeOutputsToAppend.forEach((sk, amt) -> {
                    appendedResultMap.put(sk, (Integer)appendedResultMap.get(sk) + amt * finalReturnedAmount);
                });
    ...
    public static int limitParallelByAspects(@NotNull AspectRecipe recipe, @NotNull OverlayedAspectHandler overlayedFluidHandler, int multiplier) {
        int minMultiplier = 0;
        ...
        for(int maxMultiplier = multiplier; minMultiplier != maxMultiplier; maxMultiplier = bin[2]) {
            overlayedFluidHandler.reset();
            int amountLeft = 0;
            Iterator var6 = recipe.getAspectOutputs().iterator();

            while(var6.hasNext()) {
                AspectStack fluidStack = (AspectStack)var6.next();
                if (fluidStack.amount > 0) {
                    if (multiplier > Integer.MAX_VALUE / fluidStack.amount) {
                        amountLeft = Integer.MAX_VALUE;
                    } else {
        ...
        do {
            ...
            if (!var6.hasNext()) {
                ...
                while(var6.hasNext()) {
                    ...
                    while(var10.hasNext()) {
                        inputFluid = (Map.Entry)var10.next();
                        AspectKey aspectKey = (AspectKey)fs.getKey();
                        AspectKey inputKey = (AspectKey)inputFluid.getKey();
                        if (aspectKey.equals(inputKey)) {
                            available += (Integer)inputFluid.getValue();
                        }
                    }
    ...
    public static AspectRecipeBuilder<?> doParallelRecipes(@NotNull AspectRecipe currentRecipe, @NotNull AspectRecipeMap<?> recipeMap,
                                                           @NotNull IItemHandlerModifiable importInventory,
                                                           @NotNull IMultipleTankHandler importFluids,
                                                           @NotNull IMultipleAspectTankHandler importAspects,
                                                           @NotNull IItemHandlerModifiable exportInventory,
                                                           @NotNull IMultipleTankHandler exportFluids,
                                                           @NotNull IMultipleAspectTankHandler exportAspects,
                                                           int parallelAmount, long maxVoltage, @NotNull IVoidableAspect voidable) {
        int multiplierByInputs = getMaxRecipeMultiplier(currentRecipe, importInventory, importFluids, importAspects, parallelAmount);
        if (multiplierByInputs == 0) {
            return null;
        } else {
            AspectRecipeBuilder<?> recipeBuilder = recipeMap.recipeBuilder().EUt(0);
            boolean voidItems = voidable.canVoidRecipeItemOutputs();
            boolean voidFluids = voidable.canVoidRecipeFluidOutputs();
            boolean voidAspects = voidable.canVoidRecipeAspectOutputs();
            int limitByOutput = limitByOutputMerging(currentRecipe, exportInventory, exportFluids, exportAspects, multiplierByInputs, voidItems, voidFluids, voidAspects);
    ...
    public static AspectRecipeBuilder<?> appendItemRecipes(@NotNull AspectRecipeMap<?> recipeMap,
                                                           @NotNull IItemHandlerModifiable importInventory,
                                                           @NotNull IItemHandlerModifiable exportInventory,
                                                           int parallelAmount, long maxVoltage, IVoidable voidable) {
        AspectRecipeBuilder<?> recipeBuilder = null;
